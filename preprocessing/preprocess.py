...
def remove_stopwords(text):
    ...
    stop_words.extend(['from', 'subject', 're', 'edu', 'use', 'www', 'com', 'http', 'also', 'say', 'get', 'go', 'still', 'well', 'pm', 'sincerely', 'net'])
...
def preprocess_emails(iteration):
    """
    ...
    data_words = list(tqdm(sent_to_words(data_bodies[(iteration*10000-10000):iteration*10000])))
    #print(data_words[:2])
    ...
    data_lemmatized = list(
        tqdm(
            lemmatize(data_words_nostops,
                      allowed_postages=['NOUN', 'ADJ', 'VERB', 'ADV'])))

    #print(data_lemmatized[:2])
    print("Lemmatization done")

    with open(f'data/lemmatized_word_list_{iteration}.pkl', 'wb') as f:
